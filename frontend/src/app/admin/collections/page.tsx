...
import { getCollections, createCollection, updateCollection, deleteCollection } from "@/services/productService";
...
export default function AdminCollectionsPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (err.response?.status === 400) {
        setErrorMsg(err.response.data.detail || "Cannot delete collection because it has linked products.");
      } else {
  ...
  return (
    <div className="p-6 max-w-5xl mx-auto">
      <h1 className="text-2xl font-bold text-gray-800 mb-6">Manage Collections</h1>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {collections.length === 0 ? (
                <tr>
                  <td colSpan={3} className="px-6 py-8 text-center text-gray-500">No collections found.</td>
                </tr>
              ) : (
                collections.map(col => (
                    ...
                    {editingId === col.id ? (
                      <>
                        ...
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => handleUpdate(col.id)} className="text-green-600 p-2">
                      ...
                      <>
                        <td className="px-6 py-4 font-medium text-gray-800">{col.name}</td>
                        <td className="px-6 py-4 text-sm text-gray-600">{col.description}</td>
                        <td className="px-6 py-4 text-right flex justify-end gap-2">
                          <button onClick={() => startEdit(col)} className="text-blue-500 hover:text-blue-700 p-2">
                            <Edit2 className="w-5 h-5" />
                          </button>
                          <button onClick={() => handleDelete(col.id)} className="text-red-500 hover:text-red-700 p-2">
